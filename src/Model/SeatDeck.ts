...
export class SeatDeck{
    deckInfo: SeatDeckInfo;
    columns: any[];
    rows: SeatRow[];
    constructor(deck: { deckDimensions: { width: any; length: number; exitRowsX: any; startWingsX: any; endWingsX: any; }; facilities: any; firstAvailableSeat: any; seats: any; }, tId: string){
// deckDimensions: {width: 11, length: 45, startWingsX: 4, endWingsX: 21, exitRowsX: Array(3)}
// deckType: "main"
// facilities: (28) [{…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}, {…}]
// firstAvailableSeat: {seatNumber: "28A", x: 1}
// seats: (29
        this.deckInfo = new SeatDeckInfo({deckDimensions:deck.deckDimensions, facilities:deck.facilities, firstAvailableSeat:deck.firstAvailableSeat});
...
// length: 4
        ...
        const _term = 0;
        const _arr: _.List<any> | null | undefined = [];
        ...
        _.each(deck.seats, _seat => this.rows[_seat.coordinates.x].setCol(_seat.coordinates.y, new SeatInfo(_seat, tId)));

        // Facillity 구성
        _.each(deck.facilities, _fac => {
            const _row = this.rows[_fac.coordinates.x];
            try{
                _row.setCol(_fac.coordinates.y, new FacillityInfo(_fac))
            }catch(ex){
                debugger;
            }
        });
        ...
        // 출구행 설정
        _.each(deck.deckDimensions.exitRowsX,_posX=>this.rows[_posX].set('exit'));
        // 출구행 설정
        let _row;
        for(var i=deck.deckDimensions.startWingsX,iMax = deck.deckDimensions.endWingsX;i<=iMax;i++){
            _row = this.rows[i];
            _row.set('wing');
...
class SeatDeckInfo{
    deckDimensions: any;
    facilities: any;
    firstAvailableSeat: any;
    constructor(param: { deckDimensions: any; facilities: any; firstAvailableSeat: any; }){
