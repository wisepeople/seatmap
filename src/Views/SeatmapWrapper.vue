...
export default {
  ...
  created() {
    ...
    axios
      .get(URLS.ORDER)
      .then(this.onSuccessOrder)
      .catch(this.onErrorOrder)
      .finally(this.onFinallyOrder);
  },
  methods: {
     ...
     * 예약 조회 성공
     */
    onSuccessOrder(resp: { status: string; data: null; }) {
      if (resp.status == "200") {
        ...
        const doMapping = (_next: {
            call: (arg0: CombinedVueInstance<Vue, { baseInfo: { isMobile: boolean; isIOS: boolean; isAndroid: boolean; }; popoverId: string; order: null; seats: null; seatData: null; currentSegment: null; flights: null; travelers: null; showSegment: boolean; isLoading: boolean; }, {
              // onChangePopoverState(_val){
              //     console.log('onChangePopoverState', _val);
              //     // this.$set(this,'popoverId',_val);
              // },
               ...
               * 예약 조회 성공
               */
              onSuccessOrder(resp: any): void;
              /**
               * 좌석 조회 성공
               */
              onSuccessSeat(_i: any, resp: any): void; onErrorSeat(_i: any, error: any): void;
              /**
               * 공통 통신 에러
               */
              onErrorOrder(error: any): void;
              /**
               * 공통 통신 에러
               */
              onError(_i: any, error: any): void;
              /**
               * 예약 조회 Finally
               */
              onFinallyOrder(resp: any): void;
              /**
               * 좌석 조회 Finally
               */
              onFinallySeat(_i: any, resp: any): void;
            }, unknown, Readonly<Record<never, any>>>, arg1: any, arg2: any, arg3: string) => void;
          }, _info: { [x: string]: any; }, _infoKey: any) => {
          _.each(_info, (_v, _k) => {
            switch (typeof _v) {
              case "object":
                  ...
                  if (regex2.test(_k)) {
                    ...
                    _.each(_v, (_v2, _i, _arr) => {
                      if (
                        !!_v2 &&
                        !!_v2.id &&
                        !!this.order.dictionaries[_key] &&
                        !!this.order.dictionaries[_key][_v2.id]
                      ) {
                        _arr[_i] = _.extend(
                          {},
                          _v2,
              ...
              case "string":
                if (regex.test(_k)) {
                  ...
                  if (!!this.order.dictionaries[_key] && !_info[_key]) {
                    _info[_key] = _.extend(
                      { code: _v },
                      this.order.dictionaries[_key][_v]
                    );
                    // _info[_key][_k] = _v;
                  }
                }
        ...
        const _flightsData = _.reduce(
          _cvtedData.air.bounds,
          (_arr, _bound, _bi) => {
            return _.reduce(
              _bound.flights,
              (_arr, _fl, _fi) => {
                _arr.push(
                  _.extend({}, _fl, this.order.dictionaries.flight[_fl.id])
                );
                return _arr;
              },
              _arr
        ...
        this.seats = [];
        this.seatData = new SegmentsData({
          length: this.flights.getList().length,
          firstIdx: 0,
        });
        this.flights.getList().forEach((_fl: { id: string; }, _i: any) => {
          //TODO options
          axios
            .get(URLS.SEAT + "?FLIGHTID=" + _fl.id)
            .then(this.onSuccessSeat.bind(this, _i))
            .catch(this.onErrorSeat.bind(this, _i))
            .finally(this.onFinallySeat.bind(this, _i));
        });
      } else {
    ...
    onSuccessSeat(_i: string|number, resp: { status: number; data: { contextualSeatmapReplyList: any[]; }; }) {
      ...
      if (
        resp.status == 200 &&
        resp.data &&
        resp.data.contextualSeatmapReplyList &&
        resp.data.contextualSeatmapReplyList[0]
      ) {
        this.seats[_i] = resp.data.contextualSeatmapReplyList[0];
        this.seatData.setData(
          _i,
          new SeatSegment(
            resp.data.contextualSeatmapReplyList[0],
            this.travelers.getCurrentId()
          )
        );
        // setTimeout(()=>{
        // // 후처리
